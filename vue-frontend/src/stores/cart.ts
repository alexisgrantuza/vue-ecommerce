...
export const useCartStore = defineStore('cart', () => {
  ...
  const initializeCart = () => {
    ...
    // Initialize userAuth here to avoid circular dependency issues
    if (!userAuth) {
  ...
  const saveCart = () => {
    ...
    if (userAuth.isAuthenticated && userAuth.user?.id) {
      userCarts.value[userAuth.user.id] = [...cartItems.value] // Create a copy
  ...
  const clearCart = () => {
    ...
    cartItems.value.length = 0 // Clear array without reassigning
  ...
  const setupWatchers = () => {
    ...
    watch(() => userAuth.isAuthenticated, async (isAuthenticated) => {
    ...
    }, { flush: 'post' }) // Use 'post' to ensure DOM updates happen first

    // Watch for user ID changes (switching users)
    watch(() => userAuth.user?.id, async (newUserId, oldUserId) => {
      await nextTick()
  ...
  return {
    // State
    cartItems,
    userCarts, // Expose userCarts for debugging if needed
